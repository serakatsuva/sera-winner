const weltradeMarkets=["FX Vol","SFX Vol","PainX","GainX","FlipX","SwitchX","BreakX","TrendX","PlusX 1","FiboX","QuadX","MAX PainX","MAX GainX"];
const $=id=>document.getElementById(id);
let payload=null;
let selected=weltradeMarkets[0];
...
const hasWeltradeResults=()=>payload?.ok===true&&payload?.status==="ai_analyzed"&&payload?.source_broker==="Weltrade"&&Array.isArray(payload?.markets);
const resultsAreFresh=()=>hasWeltradeResults()&&ageMinutes(payload.updated_at)<130&&ageMinutes(payload.source_updated_at||payload.updated_at)<130;

$("refreshButton").addEventListener("click",()=>loadSignals(true));
$("marketSelect").addEventListener("change",event=>{selected=event.target.value;renderSelected();});
$("copySignal").addEventListener("click",copySignal);
$("shareSignal").addEventListener("click",shareSignal);

function fillMarketSelect(){
  $("marketSelect").innerHTML=weltradeMarkets.map(name=>`<option value="${name}">${name}</option>`).join("");
...
async function loadSignals(manual=false){
  ...
  try{
    ...
    payload={ok:false,status:"load_error",source_broker:"Weltrade",note:"Impossible de charger les résultats IA."};
  }finally{
...
function render(){
  ...
  $("sourceName").textContent=payload?.source||"Weltrade MT5";
  $("modelName").textContent=payload?.model||"Luna + Sol";

  if(!hasWeltradeResults()){
    setMarketStatus("Weltrade MT5 : données requises","error");
    setAiStatus("OpenAI : en attente des bougies","error");
    notice.className="notice warning";
    notice.textContent=payload?.note||"ATTENDRE — les bougies Weltrade MT5 H1/H4 ne sont pas encore reçues. Aucun cours ni signal n’est inventé.";
    renderWaitingCards(results);
    renderSelected();
    return;
  }

  setMarketStatus(fresh?"Weltrade MT5 : données H1/H4":"Weltrade MT5 : données anciennes",fresh?"live":"error");
  ...
  notice.textContent=fresh
    ?`${payload.markets_count} instruments Weltrade analysés. BUY/SELL exige l’accord du moteur technique et d’OpenAI; l’exécution reste entièrement manuelle.`
    :"Les données ou la validation IA sont trop anciennes. Tous les verdicts restent sur ATTENDRE jusqu’à une nouvelle exportation MT5.";
...
function renderWaitingCards(container){
  weltradeMarkets.forEach(name=>{
    const card=document.createElement("button");
    card.className=`result-card${name===selected?" selected":""}`;
    card.innerHTML=`<div class="result-top"><div><h3>${name}</h3><p class="symbol">WELTRADE MT5 · H1/H4</p></div><span class="signal wait">ATTENDRE</span></div><div class="result-score"><strong>—</strong><small>Confiance IA</small></div><div class="result-bar"><i style="width:0%"></i></div>`;
    card.onclick=()=>{selected=name;$("marketSelect").value=name;renderSelected();highlightSelected();};
...
function resultCard(row,fresh){
  ...
  card.onclick=()=>{selected=row.market;ensureMarketOption(row.market);$("marketSelect").value=selected;renderSelected();document.querySelector(".analysis-grid").scrollIntoView({behavior:"smooth",block:"start"});};
  return card;
}

function renderSelected(){
  $("selectedMarket").textContent=selected;
  const row=hasWeltradeResults()?payload.markets.find(item=>item.market===selected):null;
  ...
  $("decisionConfidence").textContent=fresh?`${row.final_confidence}% de confiance`:"Données MT5 requises";
  $("decisionSummary").textContent=fresh&&row?.ai_summary?row.ai_summary:`Sera attend les bougies Weltrade H1/H4 et une validation OpenAI récente pour ${selected}.`;
  $("livePrice").textContent=row?fmt(row.price):"—";
  $("liveChange").textContent=row?`${row.technical_verdict} technique`:"Weltrade MT5 · attente";
  ...
  $("checks").innerHTML=[...confirmations.map(text=>`<div class="check"><i></i><span>${escapeHtml(text)}</span></div>`),...contradictions.map(text=>`<div class="check no"><i></i><span>${escapeHtml(text)}</span></div>`)].join("")||'<div class="check no"><i></i><span>Bougies Weltrade et analyse OpenAI récente requises</span></div>';
...
async function shareSignal(){const text=currentSignalText();if(!text)return;if(navigator.share)await navigator.share({title:"Signal Weltrade — Sera Indicator",text,url:location.href});else await navigator.clipboard.writeText(text);}
